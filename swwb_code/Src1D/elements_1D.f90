...
!!! int nabla phi_sigma * phi_sigma'
    ...
    REAL(DP), DIMENSION(:,:,:,:), POINTER :: phiphigradphi=> NULL()
    !*phi*phi*gradphi
    !*phiphigradphi(e%nsommets,e%nsommets,e%nsommets,n_dim)
...
CONTAINS
  ...
  REAL(dp) FUNCTION base_element(e,k,x) ! basis functions
    ...
    CASE(14) ! P4 Gauss Lobatto
       ...
       CASE(3)
          base_element = (x(1)*(x(1)-1._dp)*(x(1)-0.5_dp)*(x(1)-beta))/(-alpha*(2._dp*alpha-1._DP)*beta*(alpha-0.5_dp))
       CASE(4)
  ...
  FUNCTION gradient_element(e,k,x) RESULT (grad) !*derivative in the real element (due to the final division by the length)
    ...
    CASE(14) !P4 Gauss Lobatto
       ...
       CASE(1)
          ...
          !fx=-3.0*x(2)*x(2)
       CASE(2)
          fx=-(- 4._dp*alpha**2*s + alpha**2 + 4._dp*alpha*s - alpha + 8._dp*s**3 - 9._dp*s**2 + 2._DP*s)/(alpha*(alpha - 1._dp))
          !fx=3.0*x(1)*x(1)
       CASE(3)
  ...
  SUBROUTINE quadrature_element(e)  ! quadrature points and weights
    ...
    CASE(11) !*ALERT, MOVING CASE 1 DOWN TO SEAPRATE IT FROM PGL1
       e%nquad=2
       nquad=e%nquad
    ...
    CASE(1,2,3)!! exact for degree 5 !*ALERT, MOVING CASE 1 HERE TO SEAPRATE IT FROM PGL1
       e%nquad=3
       nquad=e%nquad
       ALLOCATE(e%quad(2,e%nquad),e%weight(e%nquad))
...
!*  END SUBROUTINE eval_coeff_element_old
  ...
  !*e%mass(i,j)-> local mass matrix int_K phi_i phi_j
  !*e%coeff(i,j) -> another matrix int_K phi_i  d_x phi_j !*NB:DERIVATIVE ON THE SECOND INDEX
  !*e%phiphigradphi(i,j,k,:) -> tensor int_K phi_i*phi_j*grad phi_k
  !*------------------------------------------------------------------------
...
!    ALLOCATE(e%coeff_b(e%nsommets,e%nsommets))
    ...
    !*Now phiphigradphi
    ALLOCATE(e%phiphigradphi(e%nsommets,e%nsommets,e%nsommets,n_dim))
    e%phiphigradphi=0._DP
    base=0._DP
    grad=0._DP
    DO indq=1,e%nquad
       !*Safe intialization
       ...
       DO indi=1,e%nsommets 
          ...
          grad(:,indi)=e%gradient(indi, e%quad(:,indq))
       END DO
       !*Actual integration
       DO indi=1,e%nsommets
          DO indj=1,e%nsommets
             DO indk=1,e%nsommets
                e%phiphigradphi(indi,indj,indk,:)=e%phiphigradphi(indi,indj,indk,:)+&
                & base(indi)*base(indj)*grad(:,indk)*e%weight(indq)   
             END DO
          END DO  
       END DO
    END DO
    e%phiphigradphi=e%phiphigradphi*e%volume
