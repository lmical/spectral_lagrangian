...
for dist in distributions:
    ...
    for order in range(2,10):
        ...
        with myFile:
            ...
            for k in range(NN):
                ...
                for im, method in enumerate(methods):
                    print("speed up factor "+method(1,2,dist).name, times[(im//N_meth)*N_meth,k]/times[im,k] )
        ...
        #    ax2.loglog(dts,[dt**(order)*errorsDeC[2]/dts[2]**(order) for dt in dts],":",label="ref %d"%(order))
            ...
            ax3.loglog(dts,errors[0,:],"-",color=colors[order],label="ADER(%d)"%(K_iter))
            ax3.loglog(dts,errors[1,:],"--",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
            ax3.loglog(dts,errors[2,:],"-.",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
            ax3.loglog(dts,errors[3,:],"-o",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
            ax3.loglog(dts,[dt**(order)*errors[0,2]/dts[2]**(order) for dt in dts],":",color=colors[order])#,label="ref %d"%(order))

            ax4.loglog(times[0,:],errors[0,:],"-",color=colors[order],label="ADER(%d)"%(K_iter))
            ax4.loglog(times[1,:],errors[1,:],"--",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
            ax4.loglog(times[2,:],errors[2,:],"-.",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
            ax4.loglog(times[3,:],errors[3,:],"-o",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
        #    ax4.loglog(dts,[dt**(order)*errorsDeC[2]/dts[2]**(order) for dt in dts],":",label="ref %d"%(order))


            ax5.loglog(dts,errors[0,:],"-",color=colors[order],label="ADER(%d)"%(K_iter))
            ax5.loglog(dts,errors[3,:],"-o",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
            ax5.loglog(dts,[dt**(order)*errors[0,2]/dts[2]**(order) for dt in dts],":",color=colors[order])#,label="ref %d"%(order))

            ax6.loglog(times[0,:],errors[0,:],"-",color=colors[order],label="ADER(%d)"%(K_iter))
            ax6.loglog(times[3,:],errors[3,:],"-o",color=colors[order])#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
        #    ax6.loglog(dts,[dt**(order)*errorsDeC[2]/dts[2]**(order) for dt in dts],":",label="ref %d"%(order))
    ...
    #    ax2.loglog(dts,[dt**(order)*errorsDeC[2]/dts[2]**(order) for dt in dts],":",label="ref %d"%(order))
        ...
        ax3.loglog(dts,errors[0,:],"--",color='black',linewidth=3)#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
        ax3.loglog(dts,errors[1,:],"-.",color='grey',linewidth=3)#,label="DeCStag(%d,%d)"%(M_sub,K_iter))

        ax4.loglog(times[0,:],errors[0,:],"--",color='black',linewidth=3)#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
        ax4.loglog(times[1,:],errors[1,:],"-.",color='grey',linewidth=3)#,label="DeCStag(%d,%d)"%(M_sub,K_iter))
    #    ax2.loglog(dts,[dt**(order)*errorsDeC[2]/dts[2]**(order) for dt in dts],":",label="ref %d"%(order))
...
#    ax1.set_title("ADER error convergence")
    ...
    fig3.savefig(folder+f"/convergence_ADER_adaptive_wc_"+dist+f"_staggered_{pr.name}.pdf")
...
#    ax2.set_title("ADER error convergence")
    ...
    fig4.savefig(folder+f"/convergence_vs_time_ADER_wc_adaptive_"+dist+f"_staggered_{pr.name}.pdf")
...
#    ax1.set_title("ADER error convergence")
    ...
    fig5.savefig(folder+f"/convergence_ADER_adaptive_oc_"+dist+f"_staggered_{pr.name}.pdf")

#    ax2.set_title("ADER error convergence")
    ax6.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    ax6.set_ylabel("Error")
    ax6.set_xlabel("Computational time")
    fig6.set_tight_layout(True)
    fig6.savefig(folder+f"/convergence_vs_time_ADER_adaptive_oc_"+dist+f"_staggered_{pr.name}.pdf")
...
tt=np.linspace(0,pr.T_fin,2**8+1)   #Plot the evolution for order 8
ader9 = ADER(-1, 9, "gaussLegendre")
tt_exact,uu_exact=ader9.ader(pr.flux, tt, pr.u0)
plt.plot(tt_exact,uu_exact[0,:])
plt.plot(tt_exact,uu_exact[1,:])
