...
\textbf{Well-balancing} (in particular $\textcolor{red}{\frac{\partial}{\partial t}\uvec{u}\equiv 0 ~ \Leftrightarrow ~ \frac{\partial}{\partial x}\boldsymbol{F}(\uvec{u})=\boldsymbol{S}(x,\uvec{u}) }$)


\begin{itemize}
\visible<2->{\item Refine a lot the mesh $\Rightarrow$ Longer computational time}
\visible<3->{\item Well-balancing} \visible<4->{$\Rightarrow$ Not easy}
\end{itemize}
...
\visible<6->{\textcolor{red}{But in general unknown}}
