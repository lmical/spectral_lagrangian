...
SUBROUTINE InitializeSourceTerm(choicetest,Mesh,VecProx)
 ...
 SUBROUTINE InitializeBathAndGrad(choicetest,Mesh,VecProx)
    ...
    IF (ALLOCATED(BathymetryAtDoFsUtils) .OR. ALLOCATED(GradBathymetryAtDoFsUtils)) THEN !*Then there is sth wrong, it means that we have already initialized these structures and the sub is being recalled a second time. We stop the execution
    ...
    ALLOCATE(BathymetryAtDoFsUtils(Mesh%Ns),GradBathymetryAtDoFsUtils(Mesh%Ns,N_dim))
