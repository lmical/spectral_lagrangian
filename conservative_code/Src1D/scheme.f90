...
!!!!!!!!!! Jump
  ...
  !*--------------------------------------------------------------------------------------
 SUBROUTINE jump(ijump, ed ,e1, e2, uu1, uu2, resJ, alpha, alpha2)
    ...
    SELECT CASE (ijump) !*Depending on the scheme we want he calls the proper function
    !*1 h defined by Remi
    !*11 h defined by Mario h=dx
    CASE(1,11) !*Jump of the gradient of the conserved variables
       CALL jump_burman( ed, e1, e2, uu1, uu2, resJ, alpha,alpha2,ijump)
    CASE default
 ...
 SUBROUTINE jump_burman(ed ,e1, e2, uu1, uu2, resJ, alpha, alpha2,ijump)
    ! e1 is the element before the edge, e2 the one after
    ! e1-> uu1, e2->uu2
    ...
    SELECT CASE (ijump) !*Definition of h
    !*1 h defined by Remi
    !*11 h defined by Mario h=dx
    CASE(1) 
